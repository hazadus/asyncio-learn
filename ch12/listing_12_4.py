"""
Робот с очередью
"""
import asyncio
import logging
from asyncio import Queue

from aiohttp import ClientSession
from bs4 import BeautifulSoup


class WorkItem:
    def __init__(self, item_depth: int, url: str):
        self.item_depth = item_depth
        self.url = url


async def worker(worker_id: int, queue: Queue, session: ClientSession, max_depth: int):
    print(f"Worker {worker_id}")
    while True:
        work_item: WorkItem = await queue.get()
        print(f"Worker {worker_id}: processing {work_item.url}...")
        await process_page(work_item, queue, session, max_depth)
        print(f"Worker {worker_id}: done processing {work_item.url}")
        queue.task_done()


async def process_page(
    work_item: WorkItem, queue: Queue, session: ClientSession, max_depth: int
):
    if work_item.item_depth == max_depth:
        print(f"Max depth reached for {work_item.url}")
        return

    try:
        response = await asyncio.wait_for(session.get(work_item.url), timeout=3)
    except Exception as ex:
        logging.exception(f"Error while requesting URL {work_item.url}")
        return

    try:
        body = await response.text()
        soup = BeautifulSoup(body, "html.parser")
        links = soup.find_all("a", href=True)

        for link in links:
            href: str = link["href"]

            if not href.startswith("https://hazadus.ru") and not href.startswith(
                "http"
            ):
                href = "https://hazadus.ru" + href
                print(f"{href=}")

            if href.startswith("https://hazadus.ru"):
                queue.put_nowait(WorkItem(work_item.item_depth + 1, href))
    except Exception as ex:
        logging.exception(f"Error while processing URL {work_item.url}")


async def main():
    start_url = "https://hazadus.ru"
    url_queue = Queue()
    url_queue.put_nowait(WorkItem(0, start_url))

    async with ClientSession() as session:
        workers = [
            asyncio.create_task(worker(i, url_queue, session, 3)) for i in range(100)
        ]
        await url_queue.join()
        [w.cancel() for w in workers]


asyncio.run(main())
