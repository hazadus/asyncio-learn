    ...
    async with ClientSession() as session:
        ...
        if products in pending:
            ...
            return web.json_response(
                {
                    "error": "Could not connect to the products service.",
                },
                status=504,
            )
            ...
            [request.cancel() for request in requests]
            logging.exception(
                "Error connecting to the products service.",
                exc_info=products.exception(),
            )
            return web.json_response(
                {
                    "error": "Error connecting to the products service.",
                },
                status=500,
            )
        else:
...
async def get_products_with_inventory(
    ...
    for done_task in inventory_done:
        if done_task.exception() is None:
            ...
            logging.exception(
                f"Can't get inventory info for product {product_id}",
